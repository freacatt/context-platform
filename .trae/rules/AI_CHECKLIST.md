...
- `src/pages/` route screens (compose UI + load data; keep thin)
- `src/components/` reusable UI components
- `src/components/ui/` generic primitives only
- `src/hooks/` state/actions hooks (logic lives here)
- `src/contexts/` global context (auth/app-wide state)
- `src/services/` side effects + integrations (Firebase, Anthropic)
