...
## Overview
The **Data Adapter** (`src/services/storage.ts`) is the core mechanism for data persistence in the Pyramid Solver application. It provides a unified interface for interacting with both **Local Storage (IndexedDB via Dexie)** and **Cloud Storage (Firebase Firestore)**.
...
In `src/services/localDB.ts`:
```typescript
class PyramidDB extends Dexie {
  projects!: Table<Project>;
  
  constructor() {
    super('PyramidSolverDB');
